...
#include "H5Writer.h"
#include "GeneModel.h"
#include "MigrationProbabilityEstimator.h"
#include <fstream>
#include <cstdio>
...
// Ladder-seq changes//
void ParseOptionsMigProb(int argc, char **argv, ProgramOptions& opt) {
    int verbose_flag = 0;
    int pbam_flag = 0;
    int gbam_flag = 0;

    const char *opt_string = "t:i:o:";
    static struct option long_options[] = {
      // long args
      {"verbose", no_argument, &verbose_flag, 1},
...
void ParseOptionsEM(int argc, char **argv, ProgramOptions& opt) {
  ...
  static struct option long_options[] = {
    ...
    {"bandProb", required_argument, 0, 'p'},
    {0,0,0,0}
  };
  int c;
  int option_index = 0;
  while (true) {
    ...
    case 'p': {
        stringstream(optarg) >> opt.bandProbabilityFile;
        break;
    }
    default: break;
    }
...
// Ladder-seq changes//
bool CheckOptionsMigProb(ProgramOptions& opt) {
...
void usage() {
  cout << "kallisto " << KALLISTO_VERSION << endl << endl
       ...
       << "    migrate       Calculates migration probabilities from Ladder-Seq data " << endl
...
void usageMigProb() {
  cout << "kallisto-ls" << KALLISTO_VERSION << endl << endl
       << "Usage: kallisto-ls migrate [arguments] FASTQ-files (default 7 pairs)" << endl << endl
       << "Required arguments:" << endl
       << "-i, --index=STRING            Filename for the kallisto index to be used for" << endl
       << "                              calculation of migration probabilities" << endl
       << "-o, --output-dir=STRING       Directory to write output to" << endl << endl;
  ...
  if (argc < 2) {
    ...
    if (cmd == "version") {
      ...
      if (!CheckOptionsEM(opt)) {
        ...
        MasterProcessor MP(index, opt, collection, model,8); // LadderSeq Changes. Band 8 signifies the merged files. This is temporary
        num_processed = ProcessReads(MP, opt);
        std::cout<<"Original Kallisto: "<<num_processed<<std::endl;
        // Original Kallisto End


        //Ladder Seq Changes//
        vector<int> num_processed_bands; //Vector containing the number of reads processed per band
        vector<MinCollector> collections; // Vector containing the minCollectors for each band
        int bandNum = 1;
        int bandCount = 0;
        while ( bandNum <= 7) {
            MinCollector collectionBand(index, opt);
            collections.push_back(collectionBand);
            MasterProcessor MPBand(index, opt, collections[bandNum-1], model,bandNum);
            int count = ProcessReads(MPBand, opt);
            num_processed_bands.push_back(count);
            bandCount += count;
            std::cout<<"Completed band : "<<bandNum<<std::endl;
            bandNum++;
        }
...
//        }
        ...
        std::vector<std::vector<double>> migrationProbabilities;

        if(opt.bandProbabilityFile==""){
            MigrationProbabilityEstimator  mpe(collections, index);
            migrationProbabilities = mpe.estimate();
            // Writing the migration probabilites to file
            plaintext_writer_migProb(opt.output + "/migProb.tsv", migrationProbabilities);
        }

        EMAlgorithm em(collection.counts,collections, index, collection, fl_means, opt, migrationProbabilities);
        ...
        ParseOptionsMigProb(argc-1, argv+1, opt);
        if (!CheckOptionsMigProb(opt)) {
          usageMigProb();
          exit(1);
        }else{
            KmerIndex index(opt);
            ...
            std::cout<<index.target_lens_.size()<<std::endl;
            ...
            MigrationProbabilityEstimator  mpe(collections, index);
            std::vector<std::vector<double>> migrationProbabilities = mpe.estimate();
      ...
      if (!CheckOptionsPseudo(opt)) {
        ...
        MasterProcessor MP(index, opt, collection, model,1); // LadderSeq Changes. Band 8 signifies the merged files. This is temporary

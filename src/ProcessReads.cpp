...
int ProcessBatchReads(KmerIndex& index, const ProgramOptions& opt, MinCollector& tc, std::vector<std::vector<int>> &batchCounts) {
  ...
  Transcriptome model;
  MasterProcessor MP(index, opt, tc, model,-1); // Ladder Seq Chages: putting bandNum as -1 since I dont know what else to do
  MP.processReads();
...
if(MP.bandNum!=8){
    //Ladder Seq Changes//
    // Changes : Always read 2 files -> pair one and pair 2
    // The files to be read depend on the bandNum
    for (int i = 0; i < 2; i += (paired) ? 2 : 1) {
      if (paired) {
//        std::cerr << "[quant] will process pair " << (i/2 +1) << ": "  << opt.files[(MP.bandNum*2)-2] << std::endl
//                  << "                             " << opt.files[(MP.bandNum*2)-1] << std::endl;
          std::cerr << "[quant] will process band " << MP.bandNum << ": "  << opt.files[(MP.bandNum*2)-2] << std::endl
                    << "                             " << opt.files[(MP.bandNum*2)-1] << std::endl;
      } else {
        std::cerr << "[quant] will process file " << i+1 << ": " << opt.files[i] << std::endl;
      }

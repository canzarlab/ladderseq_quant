...
public:
  ...
  SequenceReader(const ProgramOptions& opt, int bandNum) :
  fp1(0),fp2(0),seq1(0),seq2(0),
  l1(0),l2(0),nl1(0),nl2(0),
  paired(!opt.single_end), /*files(opt.files),*/
  f_umi(new std::ifstream{}),
  current_file(0), state(false), readbatch_id(-1), bandNum(bandNum) {
      //Ladder Seq Changes
//        std::cout<<"SequenceReader constructor"<<std::endl;
      if(bandNum!=8){
          for(auto it = (opt.files.begin()+((bandNum*2)-2)); it != (opt.files.begin()+((bandNum*2))); it++){
              this->files.push_back(*it);
          }
      }else {
          for(auto it = (opt.files.begin()); it != (opt.files.end()); it++){
              this->files.push_back(*it);
          }
      }
  }
  ...
  paired(false), 
  f_umi(new std::ifstream{}),
  current_file(0), state(false), readbatch_id(-1), bandNum(0) {}
...
class MasterProcessor {
public:
  MasterProcessor (KmerIndex &index, const ProgramOptions& opt, MinCollector &tc, const Transcriptome& model, int bandNum)
    : tc(tc), index(index), model(model), bamfp(nullptr), bamfps(nullptr), bamh(nullptr), opt(opt), SR(opt,bandNum), numreads(0)
    ,nummapped(0), num_umi(0), bufsize(1ULL<<23), tlencount(0), biasCount(0), maxBiasCount((opt.bias) ? 1000000 : 0), last_pseudobatch_id (-1) {

      // Ladder Seq Changes: Initialising the band number and the SequenceReader with the band Number
//      std::cout<<"MasterProcessor constructor"<<std::endl;

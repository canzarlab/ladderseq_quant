...
//Ladder Seq Changes
WeightMap_Band calc_weights_band(
  const std::vector<std::vector<int>>& counts_band,
  const EcMap& ecmap,
  const std::vector<double>& eff_lens)
{

  // weights are stored _exactly_ in the same orientation as the ec map
  WeightMap_Band weights_band(ecmap.size());

  for (size_t ec = 0; ec < ecmap.size(); ec++) {
    auto& v = ecmap[ec];
    weights_band[ec] = std::vector<std::vector<double>>(7);
    for(int band = 0; band<7 ; band++){
        std::vector<double> trans_weights;
        trans_weights.reserve(v.size());

        for (auto& trans_id : v) {
          trans_weights.push_back( static_cast<double>(counts_band[band][ec]) /
                                   eff_lens[trans_id] );
        }
        weights_band[ec][band] = trans_weights;
    }
  }

  return weights_band;
}
//Ladder Seq Changes Ends
